...
angular.module("myAppD",[])
    .controller("myCtrD",["$scope","$http","$filter",function($scope,$http,$filter) {
        ...
        var toIcon = new BMap.Icon("images/dingwei.png",new BMap.Size(33, 41), {
            anchor: new BMap.Size(5, 10),		//图标的定位点相对于图标左上角的偏移值。
            imageSize:new BMap.Size(1, 1)		//图标的大小
        });
        /**
         * 仓库图标
         * @type {BMap.Icon}
                ...
                $.ajax({
                    ...
                    success: function(userInfo){
                        ...
                        if (userInfo.errorinfo==null){
                            ...
                            if (userInfo.data.trailList.length>2){
                                ...
                                var polyline = new BMap.Polyline([
                                    new BMap.Point(userInfo.data.trailList[0].longitude, userInfo.data.trailList[0].latitude),
                                    new BMap.Point(userInfo.data.trailList[1].longitude, userInfo.data.trailList[1].latitude),
                                ], {strokeColor:colorList[cLt], strokeWeight:2, strokeOpacity:0.5});

                                map.addOverlay(polyline);
                            }else if(userInfo.data.trailList.length==0){
